<template>
    <div class="p-6">
        <h1 class="text-2xl font-bold mb-4">
            Análise de Campanhas<span v-if="loading" class="dot-anim ml-1"></span>
        </h1>

        <!-- Mostrar apenas o título com animação durante o loading -->
        <div v-if="loading" class="text-gray-500">
            A carregar dados da campanha<span class="dot-anim ml-1"></span>
        </div>

        <div v-else>
            <div v-if="!campaigns.length" class="text-gray-500">
                Ainda não tem campanhas para utilizar os algoritmos.
            </div>


            <div v-if="campaigns.length" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 font-medium">Selecione uma campanha:</label>
                        <select v-model="selectedCampaignId" class="form-control w-full max-w-xs">
                            <option disabled value="">-- Escolha uma campanha --</option>
                            <option v-for="campaign in campaigns" :key="campaign.id" :value="campaign.id">
                                {{ campaign.name }}
                            </option>
                        </select>
                    </div>

                    <div>
                        <label class="block mb-1 font-medium">Selecione um algoritmo:</label>
                        <select v-model="selectedAlgorithm" class="form-control w-full max-w-xs">
                            <option disabled value="">-- Escolha um algoritmo --</option>
                            <option value="rfm">Segmentação RFM</option>
                            <option value="churn">Previsão de Churn</option>
                        </select>
                    </div>
                </div>

                <AlgorithmWizard v-if="selectedCampaignId && selectedAlgorithm" :campanha-id="selectedCampaignId"
                    :algoritmo="selectedAlgorithm" @valido="handleValid" />
                <div class="flex gap-4 mt-2">
                    <button class="executar-btn"
                        :disabled="!valid || !selectedCampaignId || !selectedAlgorithm || loading"
                        @click="runAlgorithm">
                        Executar algoritmo
                    </button>
                    <button class="visualizar-btn" :disabled="!selectedCampaignId || !selectedAlgorithm"
                        @click="fetchResults">
                        Visualizar resultados
                    </button>

                </div>
                <div v-if="errorMessage" class="text-red-600 font-medium mt-4">
                    {{ errorMessage }}
                </div>

                <div v-if="results.length > 0 && showResults" class="overflow-x-auto mt-6 pb-32"
                    style="max-height: 500px; overflow-y: auto">
                    <div class="mb-2 text-sm text-gray-600">{{ descricao }}</div>
                    <table class="min-w-full table-auto border border-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                <th v-for="(value, key) in results[0]" :key="key" class="px-4 py-2 border">
                                    {{ key }}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(item, index) in results" :key="index">
                                <td v-for="(value, key) in item" :key="key" class="px-4 py-2 border">
                                    {{ value }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</template>

<script setup>
import { ref, onMounted, watch } from 'vue'
import axios from 'axios'
import { useToast } from 'vue-toastification'
import AlgorithmWizard from './AlgorithmWizard.vue'

const campaigns = ref([])
const selectedCampaignId = ref('')
const selectedAlgorithm = ref('rfm')
const results = ref([])
const descricao = ref('')
const loading = ref(false)
const errorMessage = ref('')
const toast = useToast()
const valid = ref(false)
const showResults = ref(false)

const fetchCampaigns = async () => {
    try {
        const res = await axios.get('http://127.0.0.1:8000/api/campaigns')
        campaigns.value = res.data || []
    } catch {
        toast.error('Erro ao carregar campanhas.')
    }
}

const handleValid = (value) => {
    valid.value = value
}

const runAlgorithm = async () => {
    if (!selectedCampaignId.value || !selectedAlgorithm.value) return

    loading.value = true
    errorMessage.value = ''
    results.value = []
    descricao.value = ''
    showResults.value = false

    try {
        await axios.post(`http://127.0.0.1:8000/api/algoritmos/gerar`, {
            campanha_id: selectedCampaignId.value,
            algoritmo: selectedAlgorithm.value
        })
        toast.success('Algoritmo executado com sucesso. Clique em "Visualizar resultados" para ver os dados.');
    } catch (err) {
        errorMessage.value = err.response?.data?.error || 'Erro ao processar dados da campanha.'
    } finally {
        loading.value = false
    }
}

const fetchResults = async () => {
    if (!selectedCampaignId.value || !selectedAlgorithm.value) return

    loading.value = true
    errorMessage.value = ''
    results.value = []
    descricao.value = ''

    try {
        const res = await axios.get(
            `http://127.0.0.1:8000/api/algoritmos/resultados/${selectedCampaignId.value}?algoritmo=${selectedAlgorithm.value}`
        )

        results.value = res.data.dados || []
        descricao.value = res.data.descricao || ''

        if (results.value.length === 0) {
            errorMessage.value = 'O algoritmo foi executado, mas não há dados suficientes para apresentar resultados.'
            showResults.value = false
        } else {
            showResults.value = true
        }
    } catch (err) {
        errorMessage.value = err.response?.data?.error || 'Erro ao buscar os resultados.'
    } finally {
        loading.value = false
    }
}

watch(selectedCampaignId, (val) => {
    if (val) localStorage.setItem('selectedCampaignId', val)
})

watch(selectedAlgorithm, (val) => {
    if (val) localStorage.setItem('selectedAlgorithm', val)
})



onMounted(() => {
    fetchCampaigns()

    const storedCampaign = localStorage.getItem('selectedCampaignId')
    const storedAlgorithm = localStorage.getItem('selectedAlgorithm')

    if (storedCampaign) selectedCampaignId.value = parseInt(storedCampaign)
    if (storedAlgorithm) selectedAlgorithm.value = storedAlgorithm
})

</script>

<style scoped>
.p-6 {
    padding: 20px;
}

.dot-anim::after {
    content: '.';
    animation: dots 1.2s steps(3, end) infinite;
}

@keyframes dots {
    0% {
        content: "";
    }

    33% {
        content: ".";
    }

    66% {
        content: "..";
    }

    100% {
        content: "...";
    }
}

.dot-anim::after {
    content: ".";
    animation: dots 1.2s steps(3, end) infinite;
}


.executar-btn {
    border: 2px solid #2563eb;
    /* azul */
    color: #2563eb;
    background-color: white;
    padding: 8px 16px;
    border-radius: 4px;
    transition: all 0.3s ease;
    font-weight: 500;
    cursor: pointer;
}

.executar-btn:hover {
    background-color: #2563eb;
    color: white;
}


.visualizar-btn {
    margin-left: 8px;
    /* ou 12px para mais espaço */
    border: 2px solid #16a34a;
    color: #16a34a;
    background-color: white;
    padding: 8px 16px;
    border-radius: 4px;
    transition: all 0.3s ease;
    font-weight: 500;
    cursor: pointer;
}


.visualizar-btn:hover {
    background-color: #16a34a;
    color: white;
}

.flex.gap-4.mt-2 {
    align-items: center;
}
</style>