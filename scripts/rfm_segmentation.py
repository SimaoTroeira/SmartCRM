...
def rfm_segmentation(base_path, empresa_id, campanha_id):
    ...
    # calcular o produto mais comprado por cliente
    try:
        # Garante que DataVenda é datetime
        df["DataVenda"] = pd.to_datetime(df["DataVenda"], errors="coerce")

        # Se não existir Quantidade, define como 1
        if "Quantidade" not in df.columns:
            df["Quantidade"] = 1

        # Agrupa: soma Quantidade e obtém a DataVenda mais recente por ClienteID + ProdutoID
        agrupado = (
            df.groupby(["ClienteID", "ProdutoID"])
            .agg(
                QuantidadeTotal=("Quantidade", "sum"),
                DataVendaMaisRecente=("DataVenda", "max")
            )
            .reset_index()
        )

        # Encontra a quantidade máxima por ClienteID
        agrupado["MaxQuantidadeCliente"] = agrupado.groupby("ClienteID")["QuantidadeTotal"].transform("max")

        # Filtra os produtos com essa quantidade máxima
        agrupado_filtrado = agrupado[agrupado["QuantidadeTotal"] == agrupado["MaxQuantidadeCliente"]]

        # Em caso de empate, escolhe o mais recente
        agrupado_final = (
            agrupado_filtrado.sort_values(["ClienteID", "DataVendaMaisRecente"], ascending=[True, False])
            .drop_duplicates(subset="ClienteID", keep="first")
        )

        # Junta com NomeProduto
        produtos_df = pd.read_json(dados_path / "produtos.json")[["ProdutoID", "NomeProduto"]]
        agrupado_final = agrupado_final.merge(produtos_df, on="ProdutoID", how="left")

        # Junta ao RFM
        produto_mais_comprado = agrupado_final.set_index("ClienteID")["NomeProduto"]
        rfm = rfm.join(produto_mais_comprado, on="ClienteID")
        rfm = rfm.rename(columns={"NomeProduto": "ProdutoMaisComprado"})

    except Exception as e:
        print(f"[AVISO]: Não foi possível calcular o produto mais comprado: {e}")
